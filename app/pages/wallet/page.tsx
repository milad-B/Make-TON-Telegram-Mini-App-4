  ...
  const copyToClipboard = useCallback(() => {
    if (tonWalletAddress) {
      navigator.clipboard.writeText(tonWalletAddress)
        .then(() => {
          // Optionally, you can show a success message or update some state
  ...
  return (
    <main className="wallet-main">
      ...
      <div className='show-rewards'>
        <p>Tokens will be send to the wallet you connected</p>
        <div>
        <Image
                  src={tokenImg}
      ...
      <div className='address-box'>
          <p>Your wallet</p>
          <h3><FaWallet opacity={0.5} size={24} /> <h4> {formatAddress(tonWalletAddress)} </h4> <FaCopy opacity={0.5} size={24} onClick={copyToClipboard} style={{ cursor: 'pointer' }} /></h3>
      </div>
      <button className='claim-button' onClick={() => sendTransaction()}>
        Claim 
      </button>
    </div>
      ) : (
